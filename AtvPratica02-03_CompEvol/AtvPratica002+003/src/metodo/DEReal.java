...
public class DEReal implements Metodo {
    ...
    @Override
    public Execucao executar() {

        Execucao exec = new Execucao(this.Np);//mu tamanho
        ...
        for (int g = 1; g <= this.gmax; g++) {
            ...
            if (populacao.getIndividuos().get(0).getFuncaoObjetivo() == 0.0 && populacao.getIndividuos().get(populacao.getIndividuos().size() - 1).getFuncaoObjetivo() != 0) {
                ...
                exec.setPior(populacao.getPiorIndividuo().clone());
            }
        ...
        exec.setMelhor(populacao.getMelhorIndividuo());
